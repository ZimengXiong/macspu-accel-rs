...
static void accel_callback(void *context, IOReturn result, void *sender, IOHIDReportType type, uint32_t reportID, uint8_t *report, CFIndex reportLength) {
  (void)context;
  (void)result;
  (void)sender;
  (void)type;
  ...
  g_decimation_count++;
  if (g_decimation_count < IMU_DECIMATION) {
    return;
  }
...
static int register_hid_devices(void) {
  ...
  while ((svc = IOIteratorNext(iter)) != 0) {
    ...
    if (usage_page == PAGE_VENDOR && usage == USAGE_ACCEL && g_device_count < MAX_DEVICES) {
      candidates++;
      ...
      if (hid) {
        if (IOHIDDeviceOpen(hid, kIOHIDOptionsTypeNone) == kIOReturnSuccess) {
          ...
          g_devices[idx] = hid;
          g_device_count++;
          IOHIDDeviceRegisterInputReportCallback(hid, g_report_bufs[idx], REPORT_BUF_SIZE, accel_callback, NULL);
...
const uint8_t *iokit_ring_ptr(void) { return g_ring; }
